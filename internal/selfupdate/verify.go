...
// Verified is an opaque result binding successful checksum and provenance
// verification of the selected release artifact to the exact staged cb.exe
// digest. A later replacement phase must re-hash the file and match this result
// before changing installed bytes.
...
// the authenticated ARM64 artifact is an archive. ghExecutable must name an
...
func (v verifier) Verify(ctx context.Context, plan Plan, binaryPath, checksumsPath, ghExecutable string) (Verified, error) {
	...
	if plan.Arch == "arm64" {
		...
		return Verified{binaryPath: extractedPath, target: target.raw, digest: extractedDigest, size: extractedSize}, nil
	}
