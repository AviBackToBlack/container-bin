...
`
	...
	for _, name := range []string{"python3", "pip", "pip3", "jq", "yq", "terraform", "ffmpeg", "node24", "npm24", "npx24", "go", "gofmt", "rustc", "cargo", "uv", "uvx"} {
...
func TestAppendMissingDefaultToolsUpgradesPreRM11(t *testing.T) {
	...
	for _, name := range []string{"node22", "npm22", "npx22", "rustc", "cargo", "uv", "uvx"} {
		if _, ok := reg.Tools[name]; !ok {
