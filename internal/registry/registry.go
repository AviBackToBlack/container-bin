...
const pipxCommand = `import fcntl
import os
import pathlib
import secrets
import shutil
...
allowed_python = pathlib.Path("/usr/local/bin/python3.13").resolve(strict=True)
lock_path = root / ".cb-pipx.lock"
with lock_path.open("a+b") as state_lock:
    fcntl.flock(state_lock, fcntl.LOCK_EX)
    status = subprocess.run(
        ["/usr/local/bin/uvx", "--from", "pipx==1.17.4", "pipx", *sys.argv[1:]],
        check=False,
    ).returncode
...
var DefaultTOML = `# container-bin tool registry
schema_version = 2
...
image = "ghcr.io/astral-sh/uv:0.12-python3.13-trixie-slim"
provider = "stateful"
command = ` + toml.Array([]string{"/usr/local/bin/python3", "-c", pipxCommand}) + `
...
# positional path mapping; explicit Windows path operands use the general mapper.
# The wrapper serializes pipx mutations and validates links after every command:
# internal links become relative and known image interpreters become copies.
# This keeps the single managed state volume portable through safe backup/restore.
